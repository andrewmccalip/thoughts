...
│   └── preferences.html     # Constants editor
│
...
The model exposes many engineering constants that can be modified via the `/preferences` page or directly in `app.py`:

- Target power capacity
- Starlink satellite specs (mass, power, array area)
- Starship payload capacity
- NGCC plant parameters
- Cost overhead fractions
...
|----------|--------|-------------|
| `/` | GET | Landing page |
| `/space-datacenters` | GET | Main analysis |
| `/preferences` | GET | Constants editor |
| `/api/references` | GET | Citation data |
| `/api/constants` | GET | Current constants |
| `/api/constants` | POST | Update constants |
| `/api/constants/reset` | POST | Reset to defaults |
...
2. **The economics are savage** — Only closes under aggressive assumptions
...
- Starlink techno-economic analysis for satellite reference data
- EIA for natural gas price forecasts
- Sargent & Lundy for datacenter capex benchmarks
- Everyone who's been nerd-sniped by this problem
...
*"If you're not doing the math, you're not contributing—you're adding noise."*
